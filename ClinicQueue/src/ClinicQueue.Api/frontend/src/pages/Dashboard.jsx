...
export default function Dashboard() {
    ...
    const fetchFilterMetadata = async () => {
        try {
            ...
            const [doctorsRes, specialtiesRes] = await Promise.all([
                ...
                axios.get('/api/doctors/specialties', { headers })
            ]);

            setDoctors(Array.isArray(doctorsRes.data) ? doctorsRes.data : []);
            setSpecialties(Array.isArray(specialtiesRes.data) ? specialtiesRes.data : []);
        } catch (error) {
            console.error('Failed to load doctor filter metadata', error);
        }
    };
    ...
    const availableDoctors = useMemo(() => {
        if (selectedSpecialty) {
            return doctors.filter(d => d.specialty === selectedSpecialty).map(d => d.name);
        }
