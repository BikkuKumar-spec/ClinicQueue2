    ...
    ILogger<VoiceController> logger) : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            // Keep voice conversations isolated from other channels like WhatsApp.
            var aiSessionId = $"voice:{request.SessionId}";
            logger.LogInformation("Voice chat request received for session {SessionId}", request.SessionId);
            ...
            logger.LogInformation(
                "Voice chat response returning to caller for session {SessionId}. Intent: {Intent}",
                request.SessionId,
                intent);
